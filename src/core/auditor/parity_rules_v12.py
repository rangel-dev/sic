...
def execute_parity_rules(
    ...
    for sku in all_skus:
        ...
        # IMPORTANTE: sem "continue" aqui. Os checks seguintes (#2, #3, #8–#12 e
        # o Check JOB) comparam o XML de catálogo/pricebook entre si e NÃO dependem
        # da grade estar anexada — devem rodar mesmo para SKUs online fora da grade.
        # Apenas os checks que exigem o Excel da marca (#1 e #4–#7) continuam
        # condicionados a `pE`/`should_compare` mais abaixo.
        if not is_offline and not is_on_grade:
            # online_excess só vale quando a marca TEM grade carregada nesta
            # execução: auditar uma marca por vez não pode marcar a outra como
            # "esquecida online" (escopo dinâmico herdado do BRD-004).
            brand_has_grade = (brand == "Natura" and has_nat) or (brand == "Avon" and has_avn)
            if brand_has_grade and not technical_skus.get(sku) and not variation_bases.get(sku):
            ...
            # sem continue: segue para os checks sistêmicos (XML vs XML) abaixo
        ...
        if not is_offline:
            ...
            # ── Check #4 (independente de grade): PREÇO AUSENTE NO SF ──
            # Produto online não pode estar sem preço, esteja ou não na grade.
            # O Check #4 clássico (abaixo, dentro de `if pE:`) cobre os SKUs da
            # grade; este ramo cobre os puramente online, olhando o XML de
            # catálogo/pricebook como um todo. O `not is_on_grade` evita disparo
            # duplo para SKUs que já caem no Check #4 clássico.
            # Exclui SKU Técnico e Variação Base (produto-pai não tem preço
            # próprio — quem precifica é a variação), mesma isenção do Check #13.
            if (not is_on_grade and not px_de and not px_por
                    and not technical_skus.get(sku)
                    and not variation_bases.get(sku)):
                err_payload = {**row_base, "de_excel": 0, "de_sf": 0,
                               "por_excel": 0, "por_sf": 0,
                               "detail": "FALTA NO SF (PREÇO)"}
                # Mesma paridade V11.6: conta como Preço E como Lista.
